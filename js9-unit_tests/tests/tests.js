...
QUnit.test( "test sum", function( assert ) {
    assert.equal(dashboard.sum(0.0000001, 0.0000002), 0.0000003, 'sum(.0000001, .0000002) = 0.0000003 true');
...
QUnit.test('test formatDateAgo', function ( assert ) {
    ...
    assert.equal(dashboard.formatDateAgo(1510000000000 - 2000), "2 seconds ago", ' formatDateAgo(NOW - 2 sec) = 2 seconds ago true');
    assert.notEqual(dashboard.formatDateAgo(1510000000000 + 2000), "in 2 seconds", ' formatDateAgo(NOW + 2 sec) !=  in 2 seconds  true');
    assert.ok(dashboard.formatDateAgo(1510000000000 - 1000000000) == "2 weeks ago", ' formatDateAgo(NOW - 1000000000 msec) == "2 weeks ago"  ok');
    assert.notOk(dashboard.formatDateAgo(1510000000000 - 10000000000000) == "20000 weeks ago", ' formatDateAgo(NOW - 10000000000000 msec) == "20000 weeks ago"  notOK');
